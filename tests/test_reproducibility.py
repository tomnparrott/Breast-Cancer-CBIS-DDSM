...
# Load the pipeline config used to find processed outputs
def _load_cfg() -> dict:
    import yaml
...
# Skip if a required file or folder is not available
...
# Rerun one indexed case and compare it with the saved probability output
@pytest.mark.slow
@pytest.mark.data
def test_inference_probability_matches_case_index_within_tolerance():
    ...
    # Choose a mid-ranked row to avoid picking an outlier
    row = df.iloc[min(10, len(df) - 1)]
    series_dir = Path(str(row["image_dir"]))
    if not series_dir.exists():
        pytest.skip(f"Series dir not found: {series_dir}")
